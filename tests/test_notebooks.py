...
def test_optimization_notebook_calibrated_full_pilot_executes(monkeypatch):
    """Diagnostic execution of both calibrated optimization-pilot cases."""
    path = Path("benchmarks/optimization.ipynb")
    notebook, _ = _read_notebook(path)
    monkeypatch.setenv("MPLBACKEND", "Agg")
    namespace = {"__name__": "optimization_calibrated_full_pilot"}
    ...
    for name, mop in (
        ...
        gt = namespace["np"].asarray(result["gt"])
        assert gt.ndim == 2
        assert gt.shape[1] == namespace["M"]
