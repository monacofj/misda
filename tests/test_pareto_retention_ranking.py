"""Tests for the experimental Pareto-retention ranking policy."""
...
def test_pareto_retention_ignores_size_as_scientific_priority():
    result = _mis_set(
    ...
    ranking = misda.rank(result, policy=misda.PARETO_RETENTION)

    assert tuple(candidate.indices for candidate in result) == before
    assert ranking.policy == "pareto_retention"
    assert ranking.indices == (2, 1, 0)
    assert ranking.selected is result[2]


def test_pareto_retention_size_only_breaks_exact_ties_operationally():
    result = _mis_set(
        _candidate(("z", "a"), (0, 1), span=9, retention=0.8),
        _candidate(("b",), (2,), span=4, retention=0.8),
        _candidate(("d",), (1,), span=10, retention=0.6),
    )
...
def test_pareto_retention_requires_stored_evidence_without_cost_opt_in():
    result = _mis_set(
    ...
    with pytest.raises(ValueError, match="requires Pareto evaluation"):
        misda.rank(result, policy=misda.PARETO_RETENTION)


def test_pareto_retention_accept_cost_authorizes_missing_evaluation(monkeypatch):
    result = _mis_set(
        _candidate(("f1", "f2"), (0, 1), span=9),
        _candidate(("f1", "f3"), (0, 2), span=4, retention=0.8),
        _candidate(("f3",), (2,), span=10, retention=1.0),
...
def test_default_policy_remains_size_span():
    ...
    assert misda.PARETO_RETENTION == "pareto_retention"
