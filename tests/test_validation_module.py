"""Tests for the MISDA discovery input and argument validation contract."""
...
@pytest.mark.parametrize(
    "value",
    [[[0.0, 1.0], [1.0, 0.0]], (0.0, 1.0, 2.0, 3.0)],
)
def test_only_arrays_and_dataframes_are_accepted(value):
    with pytest.raises(TypeError, match="must be a numpy.ndarray or pandas.DataFrame"):
        _validation.normalize_input_matrix(value)


@pytest.mark.parametrize("value", [np.arange(4.0), np.zeros((4, 2, 1))])
def test_input_must_be_two_dimensional(value):
    with pytest.raises(ValueError, match="two-dimensional matrix"):
        _validation.normalize_input_matrix(value)


@pytest.mark.parametrize("value", [np.empty((0, 2)), np.empty((4, 0))])
...
def test_small_input_is_allowed_when_fisher_z_is_not_required():
    normalized = _validation.normalize_input_matrix(
        np.ones((3, 2)), require_fisher_z=False
    )
...
def test_dataframe_error_identifies_every_non_numeric_column():
    ...
    with pytest.raises(TypeError, match=r"non-numeric columns: 'kind', 'flag'\."):
...
def test_constant_matrices_are_recorded_not_rejected():
